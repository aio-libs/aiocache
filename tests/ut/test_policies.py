...
class TestLRUPolicy:
    ...
    def test_setup(self, lru_policy):
        assert lru_policy.dq.maxlen == 10
    ...
    async def test_post_get(self, lru_policy):
        lru_policy.dq.appendleft(pytest.KEY)
        lru_policy.dq.appendleft(pytest.KEY_1)

        await lru_policy.post_get(pytest.KEY)

        assert lru_policy.dq.index(pytest.KEY) == 0
    ...
    async def test_post_set_full(self, lru_policy):
        lru_policy.dq.extend(range(1, 11))
        await lru_policy.post_set(pytest.KEY, "value")

        lru_policy.client.delete.assert_called_with(10)
        assert lru_policy.dq.index(pytest.KEY) == 0
        assert lru_policy.dq.index(1) == 1
        assert len(lru_policy.dq) == 10
